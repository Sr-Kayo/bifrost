---
title: "Alerting"
description: "Configure Bifrost Enterprise alert channels, rules, history retention, and webhook network controls in config.json"
icon: "bell"
---
...
|-------|---------|-------------|
| `allow_http` | `false` | Allow alert webhooks to use `http://` URLs. By default, only `https://` URLs are accepted. |
| `allow_private_network` | `false` | Allow webhook destinations on private, loopback, or link-local addresses. Useful for local testing only. |
